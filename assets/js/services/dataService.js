/* ==========================================================================
   GG Learning Labs — Mock domain services

   Every service returns a Promise and does the query work (search/sort/page)
   inside the service, not the component — so a real database can replace this
   without the UI changing. Writes persist to localStorage under ggl.overrides.
   ========================================================================== */
(function (GGL) {
  'use strict';
  ...
  var courseService = collection('courses', function () { return GGL.data.courses; },
    ['title', 'category', 'instructor', 'level', 'delivery']);
  var batchService = collection('batches', function () { return GGL.data.batches; },
    ['name', 'programme', 'trainer', 'location', 'department']);
  var trainerService = collection('trainers', function () { return GGL.data.trainers; },
    ['name', 'email', 'specialism', 'location', 'type']);
  var sessionService = collection('sessions', function () { return GGL.data.sessions; },
    ['title', 'batchName', 'trainer', 'location']);
  var attendanceService = collection('attendance', function () { return GGL.data.attendance; },
    ['name', 'employeeId', 'department', 'sessionTitle', 'batchName']);

  trainerService.stats = function () {
    return U.delay(340).then(function () {
      var rows = hydrate('trainers', GGL.data.trainers);
      var eff = hydrate('effRecords', GGL.data.effRecords);
      return {
        total: rows.length,
  ...
  attendanceService.summary = function () {
    return U.delay(300).then(function () {
      var rows = hydrate('attendance', GGL.data.attendance);
      var by = U.groupBy(rows, 'status');
      var total = rows.length || 1;
      return {
        total: rows.length,
        present: (by.Present || []).length, late: (by.Late || []).length,
  ...
  var notificationService = collection('notifications', function () { return GGL.data.notifications; },
    ['title', 'body', 'type']);
  notificationService.unreadCount = function () {
    return hydrate('notifications', GGL.data.notifications).filter(function (n) { return !n.read; }).length;
  };
  notificationService.markAllRead = function () {
    return U.delay(360).then(function () {
      var rows = hydrate('notifications', GGL.data.notifications);
      rows.forEach(function (n) { if (!n.read) recordUpdate('notifications', n.id, { read: true }); });
      return { updated: rows.length };
    });
  };

  var observationService = collection('observations', function () { return GGL.data.observations; },
    ['trainer', 'observer', 'sessionTitle', 'batchName']);

  /* ---------------- Assessments ---------------- */
  var assessmentService = collection('assessments', function () { return GGL.data.assessments; },
    ['title', 'course', 'category', 'type']);
  var attemptService = collection('attempts', function () { return GGL.data.attempts; },
  ...
  assessmentService.availableFor = function (userId) {
    return U.delay(400).then(function () {
      var taken = hydrate('attempts', GGL.data.attempts)
        .filter(function (t) { return t.userId === userId; })
        .map(function (t) { return t.assessmentId; });
      return hydrate('assessments', GGL.data.assessments)
  ...
  assessmentService.attemptsFor = function (userId) {
    return U.delay(360).then(function () {
      return hydrate('attempts', GGL.data.attempts).filter(function (t) { return t.userId === userId; });
    });
  };

  assessmentService.stats = function () {
    return U.delay(320).then(function () {
      var rows = hydrate('assessments', GGL.data.assessments);
      var attempts = hydrate('attempts', GGL.data.attempts);
  ...
  assessmentService.submitAttempt = function (assessmentId, answers) {
    return U.delay(700).then(function () {
      var assessment = hydrate('assessments', GGL.data.assessments)
        .filter(function (a) { return a.id === assessmentId; })[0];
      if (!assessment) throw new Error('Assessment not found.');
      var user = GGL.services.auth.getUser();
      ...
      var attempt = {
        id: U.uid('att'), assessmentId: assessment.id, assessment: assessment.title,
      ...
      function award(type, reason) {
        var rule = GGL.data.pointRules[type];
        if (!rule) return;
      ...
      if (passed && assessment.stage === 'Post' && assessment.courseId) {
        var already = hydrate('certificates', GGL.data.certificates).some(function (c) {
        ...
        if (!already) {
          ...
          certificate = {
            id: U.uid('crt'), serial: 'GGL-' + Math.floor(2000 + Math.random() * 7000),
  ...
  /* ---------------- Certificates ---------------- */
  var certificateService = collection('certificates', function () { return GGL.data.certificates; },
    ['name', 'employeeId', 'course', 'serial', 'department']);

  certificateService.forUser = function (userId) {
    return U.delay(380).then(function () {
      return hydrate('certificates', GGL.data.certificates)
  ...
  certificateService.stats = function () {
    return U.delay(300).then(function () {
      var rows = hydrate('certificates', GGL.data.certificates);
      var by = U.groupBy(rows, 'status');
      var n = new Date();
      return {
        total: rows.length, issued: (by.Issued || []).length,
  ...
  function contextFor(userId) {
    var attempts = hydrate('attempts', GGL.data.attempts)
      .filter(function (t) { return t.userId === userId; })
      .map(function (t) {
        var a = GGL.data.assessments.filter(function (x) { return x.id === t.assessmentId; })[0];
        return Object.assign({}, t, { mandatory: a ? a.mandatory : false });
      });
    var certificates = hydrate('certificates', GGL.data.certificates)
      .filter(function (c) { return c.userId === userId && c.status === 'Issued'; });
    var points = hydrate('points', GGL.data.points).filter(function (p) { return p.userId === userId; });
    return { attempts: attempts, certificates: certificates, ledger: points,
      points: U.sum(points, 'points') };
  }

  var gamificationService = {
    leaderboard: function () {
      return U.delay(460).then(function () {
        var ledger = hydrate('points', GGL.data.points);
        var byUser = U.groupBy(ledger, 'userId');
        var rows = Object.keys(byUser).map(function (userId) {
          var entries = byUser[userId];
          var user = GGL.data.users.filter(function (u) { return u.id === userId; })[0];
          var ctx = contextFor(userId);
          return {
            id: userId, userId: userId, name: entries[0].name,
            ...
            badges: GGL.data.badgeCatalogue.filter(function (b) { return b.test(ctx); }).length
          };
        });
    ...
    standing: function (userId) {
      return gamificationService.leaderboard().then(function (rows) {
        ...
        var ctx = contextFor(userId);
        return {
          rank: me ? me.rank : rows.length + 1, total: rows.length,
    ...
    badgesFor: function (userId) {
      return U.delay(300).then(function () {
        var ctx = contextFor(userId);
        return GGL.data.badgeCatalogue.map(function (b) {
    ...
    challengesFor: function (userId) {
      return U.delay(300).then(function () {
        var ctx = contextFor(userId);
        var progress = {
          ch1: ctx.attempts.filter(function (a) { return a.mandatory && a.passed; }).length,
          ch2: ctx.attempts.length, ch3: ctx.certificates.length, ch4: 4
        };
        return GGL.data.challenges.map(function (c) {
    ...
    breakdown: function () {
      return U.delay(320).then(function () {
        var by = U.groupBy(hydrate('points', GGL.data.points), 'type');
        return Object.keys(by).map(function (type) {
          return { label: (GGL.data.pointRules[type] || { label: type }).label,
            value: U.sum(by[type], 'points') };
        }).sort(function (a, b) { return b.value - a.value; });
      });
    }
  };

  /* ---------------- TOF + effectiveness calculator ---------------- */
  var tofService = collection('tofRecords', function () { return GGL.data.tofRecords; },
    ['trainer', 'evaluator', 'topic']);

  tofService.instrument = function () {
    return { sections: GGL.data.tofSections, scale: GGL.data.tofScale, max: GGL.data.tofMax };
  };
  tofService.score = function (ratings) { return GGL.data.scoreTof(ratings); };

  tofService.submit = function (payload) {
    return U.delay(650).then(function () {
      var scored = GGL.data.scoreTof(payload.ratings);
      var row = Object.assign({}, payload, {
        id: U.uid('tof'), sections: scored.sections, score: scored.overall,
      ...
      if (eff) {
        ...
        var out = GGL.data.calcEffectiveness(Object.assign({}, eff, patch));
  ...
  var effectivenessService = collection('effRecords', function () { return GGL.data.effRecords; },
    ['trainer', 'month', 'rating']);

  effectivenessService.weights = function () { return GGL.data.effWeights; };
  effectivenessService.calculate = function (m) { return GGL.data.calcEffectiveness(m); };

  effectivenessService.saveRecord = function (id, metrics) {
    return U.delay(540).then(function () {
      var out = GGL.data.calcEffectiveness(metrics);
  ...
  effectivenessService.summary = function () {
    return U.delay(360).then(function () {
      var rows = hydrate('effRecords', GGL.data.effRecords);
  ...
  /* ---------------- TNA / competency ---------------- */
  var tnaService = collection('tnaRecords', function () { return GGL.data.tnaRecords; },
    ['competency', 'department', 'category', 'recommendedCourse', 'intervention']);

  tnaService.summary = function () {
    return U.delay(340).then(function () {
      var rows = hydrate('tnaRecords', GGL.data.tnaRecords);
      var by = U.groupBy(rows, 'priority');
      return {
        total: rows.length, critical: (by.Critical || []).length, high: (by.High || []).length,
  ...
  var competencyService = collection('competencyRecords', function () { return GGL.data.competencyRecords; },
    ['name', 'employeeId', 'competency', 'department', 'role']);

  competencyService.library = function () {
    return U.delay(280).then(function () { return GGL.data.competencyLibrary.slice(); });
  };
  competencyService.profileFor = function (userId) {
    return U.delay(340).then(function () {
      return hydrate('competencyRecords', GGL.data.competencyRecords)
        .filter(function (r) { return r.userId === userId; });
    });
  };
  competencyService.heatmap = function () {
    return U.delay(400).then(function () {
      var rows = hydrate('competencyRecords', GGL.data.competencyRecords);
      var depts = Object.keys(U.groupBy(rows, 'department')).sort();
      return GGL.data.competencyLibrary.map(function (c) {
        return {
          competency: c.name, category: c.category,
  ...
  /* ---------------- Content / SOP / paths ---------------- */
  var contentService = collection('content', function () { return GGL.data.content; },
    ['title', 'course', 'category', 'type', 'author']);
  var sopService = collection('sops', function () { return GGL.data.sops; },
    ['title', 'code', 'category', 'owner', 'purpose']);
  var pathService = collection('paths', function () { return GGL.data.paths; },
    ['name', 'category', 'owner']);
  ...
  /* ---------------- Coaching / mentoring ---------------- */
  var coachingService = collection('coaching', function () { return GGL.data.coaching; },
    ['coachee', 'coach', 'focus', 'department']);
  var mentoringService = collection('mentoring', function () { return GGL.data.mentoring; },
    ['mentee', 'mentor', 'area', 'department']);

  coachingService.logSession = function (id) {
    return U.delay(480).then(function () {
      var row = hydrate('coaching', GGL.data.coaching).filter(function (r) { return r.id === id; })[0];
  ...
  /* ---------------- Requests ---------------- */
  var requestService = collection('requests', function () { return GGL.data.requests; },
  ...
  requestService.mine = function (userId) {
    return U.delay(360).then(function () {
      return hydrate('requests', GGL.data.requests).filter(function (r) { return r.requesterId === userId; });
    });
  };
  requestService.summary = function () {
    return U.delay(320).then(function () {
      var rows = hydrate('requests', GGL.data.requests);
      var by = U.groupBy(rows, 'status');
      return {
        total: rows.length,
        ...
        byStatus: GGL.data.requestStates.map(function (st) {
          return { label: st, value: (by[st] || []).length };
        }),
  ...
  /* ---------------- Newsfeed ---------------- */
  var feedService = collection('posts', function () { return GGL.data.posts; },
    ['title', 'body', 'author', 'category']);

  feedService.timeline = function (q) {
    return U.delay(420).then(function () {
      var rows = hydrate('posts', GGL.data.posts);
      ...
      var comments = hydrate('comments', GGL.data.comments);
  ...
  feedService.commentsFor = function (postId) {
    return U.delay(300).then(function () {
      return hydrate('comments', GGL.data.comments)
  ...
  feedService.toggleLike = function (postId) {
    return U.delay(220).then(function () {
      ...
      var post = hydrate('posts', GGL.data.posts).filter(function (p) { return p.id === postId; })[0];
      if (!post) throw new Error('Post not found.');
      var likedBy = (post.likedBy || []).slice();
      var idx = likedBy.indexOf(user.id);
      var liked;
  ...
  feedService.publish = function (payload) {
    return U.delay(620).then(function () {
      ...
      if (user.role === GGL.ROLES.END_USER) {
        throw new Error('Only administrators can publish to the newsfeed.');
      }
  ...
  /* ---------------- Audit / settings ---------------- */
  var auditService = collection('audit', function () { return GGL.data.audit; },
    ['actor', 'label', 'module', 'target', 'action']);

  auditService.summary = function () {
    return U.delay(320).then(function () {
      var rows = hydrate('audit', GGL.data.audit);
      return {
        total: rows.length,
  ...
  var settingsService = {
    get: function () {
      return U.delay(300).then(function () {
        ...
        return o ? Object.assign({}, GGL.data.platformSettings, o) : U.clone(GGL.data.platformSettings);
      });
    },
    save: function (group, values) {
      return U.delay(520).then(function () {
        var current = U.store.get('platformSettings', U.clone(GGL.data.platformSettings));
    ...
    reset: function () {
      return U.delay(360).then(function () {
        ...
        return U.clone(GGL.data.platformSettings);
      });
    }
  };
  ...
  var reportService = {
    catalogue: function () {
      return U.delay(360).then(function () { return GGL.data.reports.slice(); });
    },
    run: function (reportId, filters) {
      return U.delay(900).then(function () {
        var report = GGL.data.reports.filter(function (r) { return r.id === reportId; })[0];
    ...
    platformStats: function () {
      return U.delay(400).then(function () {
        var users = hydrate('users', GGL.data.users);
        var courses = hydrate('courses', GGL.data.courses);
        var batches = hydrate('batches', GGL.data.batches);
        var eff = GGL.data.effectiveness;
        return {
          admins: users.filter(function (u) { return u.role === GGL.ROLES.ADMIN; }).length,
          ...
          effectiveness: Math.round(U.avg(eff, 'overall')),
          satisfaction: Math.round(U.avg(eff, 'satisfaction')),
          trainers: GGL.data.trainers.length
        };
      });
    },
    effectiveness: function () {
      return U.delay(420).then(function () {
        var rows = GGL.data.effectiveness;
        return {
          rows: rows, overall: Math.round(U.avg(rows, 'overall')),
    ...
    activity: function (limit) {
      return U.delay(340).then(function () { return GGL.data.activity.slice(0, limit || 8); });
    },
    series: function (key) {
      return U.delay(300).then(function () {
        var s = GGL.data.series[key];
  ...
  var learningService = {
    myLearning: function () {
      return U.delay(420).then(function () { return hydrate('myLearning', GGL.data.myLearning); });
    },
    continueLearning: function () {
      return U.delay(380).then(function () {
        return hydrate('myLearning', GGL.data.myLearning)
    ...
    upcoming: function (limit) {
      return U.delay(360).then(function () {
        return hydrate('sessions', GGL.data.sessions)
    ...
    progress: function () {
      return U.delay(320).then(function () {
        var rows = hydrate('myLearning', GGL.data.myLearning);
        return {
    ...
    competencies: function () {
      return U.delay(340).then(function () { return GGL.data.competencies.slice(); });
    },
    badges: function () { return U.delay(300).then(function () { return GGL.data.badges.slice(); }); },
    advance: function (enrolmentId, delta) {
      return U.delay(480).then(function () {
        var row = hydrate('myLearning', GGL.data.myLearning)
  ...
  GGL.services.users = userService;
  GGL.services.courses = courseService;
  GGL.services.batches = batchService;
  GGL.services.trainers = trainerService;
  GGL.services.sessions = sessionService;
  GGL.services.attendance = attendanceService;
  GGL.services.notifications = notificationService;
  GGL.services.observations = observationService;
  GGL.services.assessments = assessmentService;
  GGL.services.attempts = attemptService;
  GGL.services.certificates = certificateService;
  GGL.services.gamification = gamificationService;
  GGL.services.tof = tofService;
  GGL.services.effectivenessCalc = effectivenessService;
  GGL.services.tna = tnaService;
  GGL.services.competency = competencyService;
  GGL.services.content = contentService;
  GGL.services.sops = sopService;
  GGL.services.paths = pathService;
  GGL.services.coaching = coachingService;
  GGL.services.mentoring = mentoringService;
  GGL.services.requests = requestService;
  GGL.services.feed = feedService;
  GGL.services.audit = auditService;
  GGL.services.platformSettings = settingsService;
  GGL.services.reports = reportService;
  GGL.services.learning = learningService;
